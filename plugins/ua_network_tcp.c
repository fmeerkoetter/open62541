...
UA_ClientConnectionTCP(UA_ConnectionConfig conf,
    ...
    do {
        ...
        if ((error == -1) && (errno__ != EINPROGRESS)) {
            connection_close(&connection);
            UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
        ...
        if (error == -1 && (errno__ == EINPROGRESS)) {
            ...
            UA_UInt32 timeSinceStart = (UA_UInt32) ((UA_DateTime_nowMonotonic() - connStart) * UA_DATETIME_TO_MSEC);
            ...
            int resultsize = select(clientsockfd + 1, NULL, &fdset,
                                    NULL, &tmptv);
    ...
    } while ((UA_DateTime_nowMonotonic() - connStart)*UA_DATETIME_TO_MSEC < timeout);
